    ...
    if (!c1.getSymboleDollar()){
        ...
        //c1 est composé d'entiers
            if(!c2.getSymboleDollar()){
                ...
                result = new Complexe(Rationnel(((c1.getReEntier()*c2.getReRationnel().getNumerateur())), c2.getReRationnel().getDenominateur()),
                                      Rationnel(((c1.getImEntier()*c2.getImRationnel().getNumerateur())), c2.getImRationnel().getDenominateur()));
            }
     }
     else{
           //c1 est est composé de Rationnels
            if(!c2.getSymboleDollar()){
                //c2 est un réel (partie imaginaire nulle)
               result = new Complexe(Rationnel(c1.getReRationnel().getNumerateur() * (int)c2.getReReel(), c1.getReRationnel().getDenominateur()),
                                     Rationnel(c1.getImRationnel().getNumerateur(), c1.getImRationnel().getDenominateur()));
            }
           if (c2.estReel()) {
               result = new Complexe(Rationnel(c1.getReRationnel().getNumerateur() * (int)c2.getReReel(), c1.getReRationnel().getDenominateur()),
                                     Rationnel(c1.getImRationnel().getNumerateur() * (int)c2.getImReel(), c1.getImRationnel().getDenominateur()));
           }
           else if (c2.estEntier()){
               result = new Complexe(Rationnel(((c2.getReEntier()*c1.getReRationnel().getNumerateur())), c1.getReRationnel().getDenominateur()),
                                     Rationnel(((c2.getImEntier()*c1.getImRationnel().getNumerateur()), c1.getImRationnel().getDenominateur())));
           }
           else {
